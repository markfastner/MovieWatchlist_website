const teamMembers = [
  ...
  function About() {
	return (
	  <div className=" bg-blue-400 mx-auto py-2  min-h-screen">
		...
		<div className="flex justify-center">
		  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
			{teamMembers.map((member, index) => (
			  <div key={index} className="border p-4 bg-blue-100 rounded-lg">
				<img className="mx-auto mb-4 " src={member.imageUrl} alt={member.name} style={{width: '150px', height: '150px'}}/>
				
				<h3 className="text-xl font-bold">
                <a href={`mailto:${member.email}`} className="flex justify-center text-xl font-bold text-sky-900">{member.name}</a>
              </h3>
				<p className="flex justify-center text-blue-300 ">{member.role}</p>
				<p className="flex justify-center mt-4">{member.bio}</p>
			  </div>
			))}
		  </div>
		</div>
